...
- **Harbor** — the central control plane: enrollment, IPAM, a KMS-backed signing CA, group
  firewall policy, staged rollouts, revocation, and an admin API + web console.
- **Pilot** — the per-host agent: a privileged parent that **supervises `nebula` as a
...
- Pull-based **off-mesh gateways** (ADR 0005): Harbor initiates nothing public — it *pulls*
  vetted candidates over leaf-pinned mTLS, so the public edge holds no CA.
...
- Group **firewall DSL** → compiler → signed firewall in the bundle, with compile-time invariants
...
- **RBAC** (admin/operator/viewer/break-glass), **dual-control** for privileged changes, and
  step-up MFA — with a bring-your-own-IdP session layer (OIDC / SAML / GitHub).
...
- **Signed, pinned bundles** — the config-signing, CA, and release-signing keys are the trust roots;
  the admin UI is a thin client, never a trust root.
...
spike/m0/            throwaway feasibility harness (netns + SoftHSM) — `make m0-*`
```

Module: `github.com/jeks313/nebula-control-plane` · Go 1.26 · overlay CIDR defaults to
