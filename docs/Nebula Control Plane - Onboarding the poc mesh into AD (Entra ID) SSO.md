...
| Harbor node | EC2 instance `i-0123456789abcdef1` — reached over **SSM**, not a public IP (see the access note) |
...
| KMS CA key | `arn:aws:kms:ca-central-1:123456789012:key/22222222-2222-2222-2222-222222222222` |
| KMS config-signing key | `arn:aws:kms:ca-central-1:123456789012:key/33333333-3333-3333-3333-333333333333` |
| Region | `ca-central-1` |
...
> HB_ID=$(terraform output -json instance_ids | jq -r .harbor)   # currently i-0123456789abcdef1
...
sudo systemd-run --uid=ec2-user --gid=ec2-user -p AmbientCapabilities=CAP_NET_BIND_SERVICE \
  ...
  -backend kms \
  -kms-ca-key-id arn:aws:kms:ca-central-1:123456789012:key/22222222-2222-2222-2222-222222222222 \
  -kms-config-key-id arn:aws:kms:ca-central-1:123456789012:key/33333333-3333-3333-3333-333333333333 \
...
- **ACME flags:** if your saved ExecStart had `-acme-domain …` (it should — that's what serves the
  ...
  ~/ncp/cf-token -acme-cache ~/ncp/acme -acme-email admin@example.com` block here too. Without it the
