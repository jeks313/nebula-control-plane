...
- ✅ **4.6** `POST /v1/heartbeat` + **typed command channel**. Mesh-only, tunnel-identity-authed (4.2). Pilot reports versions / cert expiry / applied bundle / clock / health; Core **upserts** it to a `heartbeats` table (one row per device — fleet visibility for 4.7) and replies with a **closed command set** (`renew` / `apply_bundle` / `restart`). `internal/heartbeat.Process` executes only known types and **refuses unknown** (never arbitrary exec); a `Reporter` loop runs in `pilot supervise -core` (a Core-issued `renew` reuses the renewal path via `Manager.RenewNow`). Core's 4.6 rule is the near-expiry **renew backstop** (`RenewCommandThreshold`); `apply_bundle` from central policy lands in M6. *Proven:* a heartbeat persists (device state queryable) and returns `[renew]` when near expiry; an **unknown command type is rejected and not executed**; an unknown overlay IP → 403.
