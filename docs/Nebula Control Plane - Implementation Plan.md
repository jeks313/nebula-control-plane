...
- **3.3a** *(seeded)* the `internal/queue.Queue` interface + in-memory impl are in place; the durable, authenticated queue (SQLite/SQS/NATS) with poison/backpressure/idempotency is still **3.3a**.
