...
- **The poc's values** (stable; the gateway/console are public DNS, harbor/lighthouse are overlay):
  ...
  - config-signing pin: from the operator, or the artifact bucket
    `https://ncp-artifacts-123456789012.s3.ca-central-1.amazonaws.com/config-signing.pub`
...
curl.exe -fsSL https://ncp-artifacts-123456789012.s3.ca-central-1.amazonaws.com/config-signing.pub -o $env:USERPROFILE\config-signing.pub
