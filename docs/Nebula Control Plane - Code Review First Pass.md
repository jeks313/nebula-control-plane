...
**Status (2026-06-18):** Largely REMEDIATED. The lint baseline is cleared to zero (`golangci-lint run` is clean and CI-enforceable), and follow-up tasks #31–#34 plus most of #35 have landed (the 7 High findings, the TOCTOU, the harbor god-file split, the dual-control committer dedup, the collector transport pooling, the adminauth/IMDS/naming fixes, and the gateway shutdown/queue-close are all fixed in code). What remains open is a handful of #35 cleanliness items — see the per-finding `(RESOLVED …)` / `(STILL OPEN …)` annotations below and the updated task table in §3.
...
> **(RESOLVED 2026-06-18 — baseline now zero, commit `c034b34`):** the 18-item baseline was cleared.
> The SA1019 crypto deprecations were migrated off (`internal/hostkey` etc. now use `crypto/ecdh`); the
> 11 `revive: exported` were satisfied with doc comments; the 3 `exitAfterDefer` got `//nolint` at the CLI
> fatal-exit sites; and `sloppyReassign` was disabled in `.golangci.yml` (false-positive on named returns).
> `golangci-lint run ./cmd/... ./internal/...` is now clean (0 issues) and CI can enforce it. Task #34 is
...
> **(STATUS 2026-06-18):** All 7 High findings are FIXED in code. Per-finding annotations below; tasks
> #31/#32/#33 (which covered the High set) are complete.
...
> **(RESOLVED 2026-06-18 — commit `de94ca8`, task #33 part 2):** `cmd/harbor/main.go` is now **548 LOC**
> and holds only `main()`/`usage()`/the dispatch switch + shared helpers. The command groups were split
> into sibling files exactly per the Fix below — `ca.go`, `enroll.go`, `policy.go`, `registry.go`,
> `rollout.go`, `serve.go` (plus the pre-existing `cloudtrust.go`/`seeddemo.go`/`admintoken.go` etc.).
...
> **(RESOLVED 2026-06-18 — commit `b6161f6`, task #33 part 1):** the dual-control commit-time validators
> were deduped into the domain packages; the 4 inline `dc.Register` closures now resolve to a single
> authoritative committer per kind, co-located with `policy.Parse`/`CheckInvariants` and `cloudtrust.Parse`.
...
> **(RESOLVED 2026-06-18 — commit `5ef9204`, task #32):** `record()` now **returns error** (current
> `internal/enrollment/enrollment.go:1049`, ending `return c.cfg.Store.DB...Create(&e).Error`) and the
> issue/pending paths treat a failed Create as a non-terminal failure rather than delivering a bundle for
> an unrecorded issuance.
...
> **(RESOLVED 2026-06-18 — commit `5ef9204`, task #32):** renew now **fails closed** on the fingerprint
> Update (current `internal/coreapi/coreapi.go:429-433`): on error it returns `wire.CodeInternal`
> ("renew bookkeeping failed") instead of delivering a renewed bundle whose fingerprint Core never recorded.
...
> **(RESOLVED 2026-06-18 — commit `4514ee6`, task #31):** the background loops now run under a
> `sync.WaitGroup` (current `cmd/pilot/main.go:518`, `wg.Add(1)` per loop, `wg.Wait()` on shutdown), so an
> in-flight renewal/drift write drains before exit.
...
> **(RESOLVED 2026-06-18 — commit `4514ee6`, task #31):** a single `applyMu sync.Mutex` (current
> `cmd/pilot/main.go:535`) is now threaded as `Locker` into renew, drift, and the apply_bundle handler, so
> all writers of the host layout are serialized.
...
> **(RESOLVED 2026-06-18 — commit `4514ee6`, task #31):** `defaults()` now wraps **all** default-filling
> (not just `restartCh`) inside `s.initOnce.Do(...)` (current `internal/supervisor/supervisor.go:79-80`,
> with a comment noting Run and Restart both call it), so the scalar/Logger writes are no longer racy.
...
### Medium (11) — partially resolved

> **(STATUS 2026-06-18):** The naming/error-handling/concurrency Mediums are FIXED
> (`signer.Policy`→`IssuePolicy`, `adminauth.New` returns an error, collector transport pooling, gateway
> shutdown join, the cross-process TOCTOU). The three *structural* Mediums (adminapi split, `coreFlags`
> god-struct, active-policy/cloudtrust accessor dedup) are STILL OPEN — see per-finding notes. The
> remaining `api-design` Mediums (bundle-config dedup, three data-access conventions, exported `store.DB`)
> are also STILL OPEN.
...
> **(STILL OPEN 2026-06-18):** Not done — and the file has GROWN to **1363 LOC**. The package gained
> further per-feature splits (`ipam_ops.go`, `releases_ops.go`) but the suggested `policy_ops.go` and
> `httpx.go` extractions were not made; the dual-control/policy/cloudtrust handler family + generic HTTP
> plumbing still live in `adminapi.go`. Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** `coreFlags` survives the god-file split (it moved to `cmd/harbor/enroll.go:42`)
> but was not broken into sub-structs (`queueFlags`/`bundleFlags`/`sourceFlags`) and the consumer/signer
> assembly was not extracted into an internal `coreboot` package. Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** No `policy.Active(ctx, dc)` / `cloudtrust.Active(ctx, dc)` accessor exists
> yet; the `LatestCommitted`+`Parse` pattern is still re-derived in cmd and adminapi. Tracked under #35.
...
> **(RESOLVED 2026-06-18 — commit `4b28d43`, #35):** renamed to `signer.IssuePolicy` (current
> `internal/signer/signer.go:61`, with a doc comment noting it avoids clashing with `policy.Policy`).
...
> **(STILL OPEN 2026-06-18):** No shared `bundle.Settings`/`BundleConfig` type extracted; the ten fields are
> still declared in both `Config` structs, kept in sync only by the prose comment. Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** The three conventions persist; `ipam.NewAllocator` still takes `*store.Store`
> (current `internal/ipam/ipam.go:149`) rather than the `*gorm.DB` it actually uses. Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** `store.Store.DB` is still an exported `*gorm.DB` (current
> `internal/store/store.go:61`); packages continue to reach through it directly. Tracked under #35.
...
> **(RESOLVED 2026-06-18 — commit `2bb1d19`, #35):** `adminauth.New` now returns `(*Service, error)` and
> threads the `newCookieSigner` error, matching its sibling constructors; the caller decides to exit.
...
> **(RESOLVED 2026-06-18 — commit `1e4cd82`, #35):** the collector now caches one `*http.Client` per
> gateway keyed by name+pin (current `internal/collect/collector.go:77` `clients map[string]*http.Client`,
> served via `clientFor`), so connections pool across cycles.
...
> **(RESOLVED 2026-06-18 — commit `89f8816`, #35):** both servers now run under a shared `sync.WaitGroup`
> with `wg.Wait()` before exit (current `cmd/gateway/main.go:190,216`; `startCollect`/`startObs` take the
> `*sync.WaitGroup`), so the collect server's in-flight drain completes before the process exits.
...
> **(RESOLVED 2026-06-18 — commit `9c6e157`, #35):** the consume is now atomic and conditional — the
> `read_count = 0` guard moved into the WHERE clause (current `internal/queue/durable.go:286-291`,
> `Where("id = ? AND status = ? AND read_count = 0", ...)`), and `RowsAffected == 0` returns `ErrResultGone`,
> so only the UPDATE that flips 0→1 wins regardless of process count.
...
### Low (11) — partially resolved

> **(STATUS 2026-06-18):** Resolved — `KindPolicyPublish` alias dropped, `policy.PolicyGroups`→`Groups`,
> gateway durable-queue Close on shutdown, IMDS NewRequest error handling. Still open — `AuditFunc` dedup
> (in fact now redeclared in even MORE packages), `enrollment.Terminal` wrapper, adjacent-param
> transposition hazards, constructor-naming consistency, PKCS#11 backend `Close` on shutdown, fail-open
> audit logging, gateway backpressure-vs-500 mapping. All open items are tracked under #35.

#### [naming] main.go re-exports policy.PublishKind as a local KindPolicyPublish const, obscuring the canonical source

> **(RESOLVED 2026-06-18 — commit `1e4cd82`, #35):** the `KindPolicyPublish` alias was deleted; call sites
> use `policy.PublishKind` directly.
...
> **(RESOLVED 2026-06-18 — commit `4b28d43`, #35):** renamed to `policy.Groups` (current
> `internal/policy/analysis.go:109`).
...
> **(STILL OPEN 2026-06-18 — got worse):** `type AuditFunc` is now declared verbatim in **7** packages
> (dualcontrol, revocation, gatewayreg, rollout, lighthouse, plus the newer `netblock` and `reaper`), still
> with no single `store.AuditFunc`/`internal/audit` source of truth. Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** Both `Terminal` and the unexported `terminal` still exist (current
> `internal/enrollment/enrollment.go:269` and `:273`). Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** `record()` still takes runs of adjacent same-type string params (current
> `internal/enrollment/enrollment.go:1049`); genesis.Run's adjacent backend params likewise unchanged.
> Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** The mixed constructor shapes persist (`ipam.NewAllocator`, the
> `rollout/lighthouse/revocation/gatewayreg` positional `New(db, audit)`, etc.). Tracked under #35.
...
> **(RESOLVED 2026-06-18 — commit `89f8816`, #35):** the gateway now `defer closeQueue(q)` (current
> `cmd/gateway/main.go:135`), type-asserting to `io.Closer` so the SQLite pool/WAL is cleanly released,
> matching the harbor subcommands.
...
> **(STILL OPEN 2026-06-18):** The `signer.Backend` interface still has no `Close()` and the daemons do not
> type-assert/defer-close the CA/config backends, so the HSM session is held for the process lifetime.
> Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** The publish path still maps every error to `CodeInternal "enrollment queue
> unavailable"` (current `internal/gateway/gateway.go:229`) without branching on `queue.ErrBackpressure`
> (→ rate-limited) or `ErrDuplicate` (→ success). Tracked under #35.
...
> **(STILL OPEN 2026-06-18):** Audit writes remain fire-and-forget (`_ = c.audit(...)` throughout
> `internal/enrollment/enrollment.go`, e.g. enroll-pending/attested/approved/denied/exhausted) with no
> WARN log when an append is dropped, so a chronic audit-write failure is still invisible. Tracked under #35.
...
> **(RESOLVED 2026-06-18 — commit `4b28d43`, #35):** both `NewRequestWithContext` calls now capture and
> return the error (current `internal/awsattest/awsattest.go:198` `tokReq, err := ...` and `:212`
> `r, err := ...`), so a malformed override fails the enrollment cleanly instead of panicking.
...
| Task | Scope | Status (2026-06-18) |
|------|-------|---------------------|
| **#31** | Pilot runtime concurrency hardening — `Supervisor.defaults()` race, renew/drift file-write race, shutdown not joining in-flight writes | **DONE** (`4514ee6`) |
| **#32** | Fix the two swallowed-error data-loss bugs (`enrollment.record()` Create, coreapi renew fingerprint Update) | **DONE** (`5ef9204`) |
| **#33** | Split `cmd/harbor/main.go` (2078 LOC) per the package convention + dedup the dual-control committer into the domain packages | **DONE** (`b6161f6` + `de94ca8`; main.go now 548 LOC) |
| **#34** | Migrate off deprecated crypto APIs (SA1019) to `crypto/ecdh` | **DONE** (subsumed by the baseline-zero pass `c034b34`) |
| **#35** | golangci-lint baseline backlog + idiom/hygiene cleanup (naming clashes, constructor consistency, AuditFunc dedup, resource Close on shutdown, TOCTOU) | **MOSTLY DONE** — baseline cleared to zero (`c034b34`); TOCTOU/collector-pool/adminauth/gateway-shutdown/queue-close/IMDS/name-clashes fixed. **Residual backlog open** (see below). |
...
**#35 residual cleanliness backlog (STILL OPEN 2026-06-18):**

- `AuditFunc` type still redeclared verbatim across 7 packages (dualcontrol, revocation, gatewayreg, rollout, lighthouse, netblock, reaper) — no single `store.AuditFunc`/`internal/audit` source of truth.
- Bundle-assembly config (10 fields) still duplicated across `coreapi.Config` + `enrollment.Config`, kept in sync only by a comment.
- `internal/adminapi/adminapi.go` not split (now 1363 LOC); `coreFlags` god-struct not decomposed; no `policy.Active`/`cloudtrust.Active` accessor (cmd/adminapi split-brain persists).
- `store.Store.DB` still an exported `*gorm.DB`; `ipam.NewAllocator` still takes `*store.Store`; constructor-naming mix unchanged.
- PKCS#11 backend `Close()` still not invoked on daemon shutdown.
- Fail-open audit appends still not logged at WARN on a dropped write.
- Gateway enroll still maps `queue.Publish` backpressure to a generic 500 instead of a rate-limit/503.
- `enrollment.Terminal` still a wrapper over an unexported `terminal()`; `record()`/`genesis.Run` adjacent same-type params unchanged.
