...
**Status (2026-06-18):** Largely BUILT & LIVE on the `poc` (which is the prod stack — Aurora + KMS CA/config-signing + ACME edge TLS + Fargate gateway/lighthouse + EC2 harbor/client/monitoring, SSM-only access). Shipped & live: Pilot + nebula self-update/distribution (ADR 0003), client install/bootstrap (ADR 0008), pull-based off-mesh enrollment gateway (ADR 0005), distroless Fargate images (ADR 0006), DB-driven IPAM with named netblocks (ADR 0010), live used%/observability, the device reaper, ephemeral-host lifecycle (first slice), and the React admin console (served with `-tags ui`). SSO user enrollment (ADR 0004) + control-plane trust-zone (ADR 0009) are CODE-COMPLETE and deploy-threaded but OFF BY DEFAULT (fail-closed until an operator wires the AD SAML app + publishes a usertrust config + sets `sso_acs_url`). Remaining prod-grade gaps: HA/multi-AZ, backups/DR for CA material + DB, real Entra SSO for the *console* (currently dev mock-idp; the console and the enrollment portal are **two separate** Entra SAML apps with distinct entity IDs — ADR 0009), WAF, distroless-hardening verification. This document is the original design rationale; section-level status is annotated inline in §11–§12.
...
> - **Build status (2026-06-18):** the v3 design has since been built out and deployed live on the `poc`/prod stack. See the Status line above and the inline annotations in §11 (roadmap) and §12 (open questions). The architecture and security rationale below (§1–§10, §13) still hold; the ADRs (0001–0010) record where reality has elaborated on or superseded specifics (e.g. the enrollment gateway is now a *pull-based, off-mesh* design per ADR 0005; the gateway and lighthouse run on Fargate per ADR 0006; IPAM is now DB-driven named netblocks per ADR 0010; user enrollment is SSO/SAML per ADR 0004 rather than the originally-sketched OIDC device flow).
...
> **Status (2026-06-18): SHIPPED & LIVE (ADR 0010).** As built, IPAM is DB-driven **named netblocks** with a `NetblockResolver`: per-join-method binding (join-key sub-range / cloud-trust netblock / SSO usertrust netblock), provenance recorded on each allocation (`ip_allocations.netblock_id` + method), auto-grow into the buddy block, genesis central(`/27`) + default reservations, growth-envelope `Suggest`, and live used% (allocated ∩ fresh-heartbeat) surfaced via the admin API + `ncp_ipam_*` Prometheus gauges + dashboard. Decision log: IPAM-DECISIONS.md (D1–D23). The pool/sub-range sketch below is the original rationale; named netblocks are its as-built realization. Optional outstanding: per-block growth-margin override, `sub_range`→netblock rename, central role-map doc.
...
- **Admin API/UI** (mesh-only + IdP SSO + MFA; RBAC; dual-control on privileged ops; reachable out-of-band for break-glass, §10): approve/deny enrollment, set group-map, author/publish policy (two-person), CA/key rotation, revoke device (bulk = dual-control), break-glass ops. *(The console's "IdP SSO" is its **own** Entra SAML app — distinct from the public enrollment-portal SSO of ADR 0004; the two are separate Entra apps with distinct entity IDs, ADR 0009.)*
...
> **Status (2026-06-18):** Phases 0–3 are SHIPPED & LIVE on the `poc`/prod stack; Phase 4 is PARTIAL; Phase 5 is PARTIAL. Inline status per phase below. Note the as-built specifics diverge from this 2026-06-11 sketch in places (KMS-backed CA + ACME edge TLS live on the `poc`; the enrollment gateway is *pull-based & off-mesh*, ADR 0005; gateway + lighthouse run on Fargate, ADR 0006; IPAM is DB-driven named netblocks, ADR 0010; user enrollment is SSO/SAML, ADR 0004, rather than OIDC device flow).

- **Phase 0 — Foundations. SHIPPED & LIVE.** Isolated PKI account + KMS CA + config-signing key; **genesis ceremony** (bootstrap script + runbook, live); cert v2 profile; Signer (with circuit-breaker) backed by KMS (the `nebula-cert-kms` pattern, validated at M0 via SoftHSM/PKCS#11); lighthouses; CIDR plan; **gateway/core split** built in from day one (now pull-based & off-mesh, ADR 0005).
- **Phase 1 — Pilot + token enrollment (MVP). SHIPPED & LIVE.** Wrapper supervises nebula (binary digest checks from day one); local keypair; join-key/token enroll via gateway; KMS signing; IPAM; mesh-based proactive renewal + Core self-renewal path. Client install/bootstrap (ADR 0008, Linux/macOS-launchd/Windows-SCM) and pilot+nebula signed waved self-update (ADR 0003) shipped and live-validated. *Kills pains #1–3.*
- **Phase 2 — Cloud attestation. PARTIAL.** Attestation framework + cloud-trust + immutable-fact group-map built and live for join-key/cloud-trust paths; **user enrollment shipped as SSO/SAML (ADR 0004)** — CODE-COMPLETE, deploy-threaded, OFF BY DEFAULT (fail-closed until an operator wires AD SAML + a usertrust config + `sso_acs_url`) — rather than the originally-sketched OIDC device flow. PrivateLink/Private Endpoint and full AWS-sigv4/Azure-attested-data auto-enroll remain to be exercised on real cloud VMs.
- **Phase 3 — Central firewall policy. SHIPPED & LIVE.** Policy-as-data; default-deny; invariants; dual-control publish; JWS bundles; canary + auto-rollback (rollout engine, `internal/rollout`); drift revert. *Kills pain #4.*
- **Phase 4 — CA & key rotation. PARTIAL.** Multi-CA trust bundles + lifecycle machinery built; **rotation drill not yet run end-to-end in staging**; HA/multi-AZ + backups/DR for CA material + DB still outstanding (ADR 0007 prod-grade gaps).
- **Phase 5 — Hardening & scale. PARTIAL.** Detection-relevant observability (Prometheus/Loki/Grafana on a monitoring node, gateway + harbor-collect metrics, data-plane stats), signed waved self-update, and the device reaper (auto-reap of lapsed hosts) are SHIPPED & LIVE. Still outstanding: posture-gated renewal; full SIEM wiring; **HA Harbor**; parser fuzzing in CI; gateway pentest; external protocol review (§13); WAF; pilot/client `/metrics` (parked, task #99).
...
- **Container / autoscaling granularity.** The model is VM-centric. Per-container Nebula identity is likely the wrong granularity — prefer **per-host Pilot with containers sharing the overlay**, unless per-workload isolation is truly required. High-churn fleets stress IPAM/quarantine; auto-reap on terminate events. *(Update 2026-06-18: the **device reaper** is SHIPPED & LIVE — runs hourly on core-api, reclaims leaked overlay IPs / prunes stale heartbeats / revokes lapsed certs past a grace, never touches control-plane/reserved groups or the central netblock; conservative defaults, dry-run, `ncp_reaper_*` metrics. Decision log: REAPER-DECISIONS.md. Outstanding: hard-delete of reaped records; tie reaping to cloud instance-terminated events and IdP-offboarding/renewal-denial.)*
...
- **Stack:** **Go** for Harbor and Pilot (matches Nebula; can import `github.com/slackhq/nebula/cert`; static binaries). **State via GORM** with a one-line dialect swap: **SQLite** (pure-Go `glebarez`, cgo-free) for minimal-footprint local dev, **Postgres** for production — same models/queries on both (decided 2026-06-12, M2). Migrations run over the GORM connection via **gormigrate** (per-dialect SQL files); golang-migrate was rejected because its sqlite driver collides with the pure-Go one (both register the `"sqlite"` name).
- **Build vs buy — DECIDED (build).** Overlaps **Defined Networking's** managed plane; the mesh-only management plane + KMS custody + account isolation were the deciding lens. We built it — the control plane is now live on the `poc`/prod stack.
