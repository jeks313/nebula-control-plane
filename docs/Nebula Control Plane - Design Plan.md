...
- **Nebula reload semantics — verify.** The zero-downtime claims assume SIGHUP hot-reload of firewall rules and possibly certs/CA bundle. Confirm exactly what reloads vs. needs a restart in the pinned Nebula version; adjust 4.4/4.5 if cert/CA changes require a brief restart.
