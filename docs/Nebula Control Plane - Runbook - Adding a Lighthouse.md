...
**Status (2026-06-24): LIVE — this exact flow added `lighthouse-2` (`10.44.0.6` @ `203.0.113.20:4242`) to the `poc`/prod stack.** lighthouse-1 was untouched; both `ncp-lighthouse` and `ncp-lighthouse-2` ECS services run `1/1 ACTIVE`.
...
below: region `ca-central-1`, prefix `ncp`, harbor `i-0123456789abcdef1`.
...
```bash
terraform output -json lighthouse_addrs    # {"lighthouse-1":"...","lighthouse-2":"203.0.113.20:4242"}
...
ssh -o "$PROXY" -i ~/.ssh/<genesis-key> ec2-user@$HB \
  '. /etc/profile.d/harbor-cli.sh; /tmp/harbor-lh lighthouse add -ip 10.44.0.6 -addrs 203.0.113.20:4242 -name lighthouse-2 -actor <you>'
```
