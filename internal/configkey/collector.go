...
// Collector is a scrape-time Prometheus source for the M8.5 config-signing-key-deletion state — the
// signal a CloudWatch/Prometheus alarm watches so an operator is paged while a signing key sits in
// its pending-deletion window (the last chance to cancel before it is destroyed). Registered on
// Core's /metrics like the CA/lighthouse/IPAM collectors.
type Collector struct {
	db  *gorm.DB
	now func() time.Time
}

// NewCollector builds a config-signing-key metrics collector over the store's DB.
func NewCollector(db *gorm.DB) *Collector { return &Collector{db: db, now: time.Now} }

var (
	descKeyDeletionPending = prometheus.NewDesc(
		"ncp_configkey_key_deletion_pending",
		"Number of config-signing keys currently scheduled for deletion (M8.5; each is in its cancellable pending window).",
		nil, nil,
	)
	descKeyDeletionSeconds = prometheus.NewDesc(
		"ncp_configkey_key_deletion_seconds_remaining",
		"Seconds until a config-signing key is destroyed (M8.5); labelled by key. Negative once the window has elapsed. Alarm on a low/expiring value to catch an unintended deletion in time to cancel.",
...
// Collect emits the pending-deletion count and, per pending key, the seconds until its key is
// destroyed. A DB error yields no samples this scrape (the gauge simply goes absent) rather than a
// panic — mirroring the CA/lighthouse collectors.
