...
// TestRBACOperator proves the 2.11 role matrix: an operator may run day-2 fleet
// ops (lighthouse/rollout/joinkey/enroll) but is forbidden the privileged surface
// (policy publish + dual-control approvals), which is admin-only.
func TestRBACOperator(t *testing.T) {
	...
	// Forbidden: policy publish is admin-only.
	if code, _ := req(t, ts, "POST", "/admin/v1/policy/propose", "otto",
		map[string]any{"policy": "allow web -> db tcp 5432\n"}); code != http.StatusForbidden {
		t.Fatalf("operator policy propose = %d, want 403", code)
	}
	// Forbidden: approving a dual-control change is admin-only (403 before any
	// not-found lookup — the permission gate runs first).
...
func TestRBACAdminSuperuser(t *testing.T) {
	...
	if code, _ := req(t, ts, "POST", "/admin/v1/policy/propose", "ada",
		map[string]any{"policy": "allow web -> db tcp 5432\n"}); code != http.StatusCreated {
		t.Fatalf("admin policy propose = %d, want 201", code)
	}
}
