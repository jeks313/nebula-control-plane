...
#
# Requires: a container engine (podman or docker), terraform, aws (+ go for the
# gateway). Override the engine with CONTAINER_ENGINE=... ; otherwise podman is
# preferred, then docker. The podman/docker CLIs are command-compatible here.
set -euo pipefail
