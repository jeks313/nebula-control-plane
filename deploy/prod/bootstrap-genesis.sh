...
# CLI convenience: drop HARBOR_DB_* env defaults into /etc/profile.d so an interactive login shell on
# the control-plane node can run `harbor <subcommand>` with NO connection flags (the binary reads
# these as its flag defaults — positionals like `enroll approve <id>` included). It's a system-wide
# profile, so it applies to every login user (ec2-user AND the SSM-created ssm-user). No DB password
# is written here — harbor fetches the rotating credential from Secrets Manager via the instance role.
echo "==> [harbor] install /etc/profile.d/harbor-cli.sh (HARBOR_DB_* env defaults + harbor-logs helper)"
if [[ "$DB_BACKEND" == "aurora" || "$DB_BACKEND" == "postgres" ]]; then
...
# Auto-generated by bootstrap-genesis.sh. Lets an interactive login shell on the control-plane node
# (ec2-user OR ssm-user) run \`harbor <subcommand>\` with no DB flags: the harbor binary reads these
# HARBOR_DB_* vars as its DB-connection flag defaults. The DB PASSWORD is NOT here — harbor fetches
# the rotating credential from Secrets Manager via the instance role. An explicit -dsn/-driver wins.
$HARBOR_CLI_ENV
# Tail the control-plane units:  harbor-logs -f   |   harbor-logs -n 100 -u ncp-admin
# Falls back to sudo when the caller isn't in the systemd-journal group (e.g. ssm-user; ec2-user is).
harbor-logs() {
  [ "\$#" -eq 0 ] && set -- -n 50
  local jc=journalctl
  id -nG 2>/dev/null | tr ' ' '\n' | grep -qx systemd-journal || jc="sudo journalctl"
  \$jc -u ncp-core -u ncp-collect -u ncp-admin -u ncp-nebula "\$@"
}
RPROF
echo "    harbor CLI env installed (run 'harbor joinkey list' etc. with no flags after re-login)"
