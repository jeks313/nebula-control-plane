...
echo "==> [harbor] install /etc/profile.d/harbor-cli.sh (HARBOR_DB_* + signing/queue env, sh-safe) + /usr/local/bin/harbor-logs"
if [[ "$DB_BACKEND" == "aurora" || "$DB_BACKEND" == "postgres" ]]; then
...
# Signing + queue + pool flag defaults for the core subcommands (enroll approve/worker), so an
# INTERACTIVE ec2-user shell (or SSM session) runs `harbor enroll approve <id> -approver A` with no
# signing/queue flags — same backend Core uses. queue/hmac are PATHS (the 0600 key files stay
# ec2-user-only, so only ec2-user can actually use these). -approver is intentionally NOT exported
# (it attributes the approval in the audit trail; a shared default would mis-attribute every approval).
if [[ -n "$CA_KEY_ARN" && -n "$CFG_KEY_ARN" ]]; then
  HARBOR_CLI_SIGN_ENV="export HARBOR_BACKEND=kms
export HARBOR_CA_CERT=/home/$SSH_USER/ncp/genesis/ca.crt
export HARBOR_KMS_CA_KEY_ID='$CA_KEY_ARN'
export HARBOR_KMS_CONFIG_KEY_ID='$CFG_KEY_ARN'${TF_REGION:+
export HARBOR_KMS_REGION='$TF_REGION'}
export HARBOR_HMAC_KEY=/home/$SSH_USER/ncp/hmac.b64
export HARBOR_QUEUE_DSN=/home/$SSH_USER/ncp/queue.db
export HARBOR_QUEUE_KEY=/home/$SSH_USER/ncp/queue.b64
export HARBOR_POOL='$POOL'"
else
  HARBOR_CLI_SIGN_ENV="export HARBOR_BACKEND=software
export HARBOR_CA_CERT=/home/$SSH_USER/ncp/genesis/ca.crt
export HARBOR_CA_KEY=/home/$SSH_USER/ncp/genesis/ca.key
export HARBOR_CONFIG_KEY=/home/$SSH_USER/ncp/genesis/config-signing.key
export HARBOR_HMAC_KEY=/home/$SSH_USER/ncp/hmac.b64
export HARBOR_QUEUE_DSN=/home/$SSH_USER/ncp/queue.db
export HARBOR_QUEUE_KEY=/home/$SSH_USER/ncp/queue.b64
export HARBOR_POOL='$POOL'"
...
# ssm-user) run \`harbor <subcommand>\` with no flags: the harbor binary reads these HARBOR_DB_* vars
# (and the HARBOR_BACKEND / HARBOR_CA_CERT / HARBOR_KMS_* / HARBOR_QUEUE_* / HARBOR_POOL signing+queue
# vars) as its flag defaults. POSIX-sh-safe (exports only) — sourced by login shells AND the SSM
# Session Manager shell profile (a non-login sh). No DB PASSWORD here — harbor fetches the rotating
# credential from Secrets Manager via the instance role. An explicit -flag always wins.
$HARBOR_CLI_ENV
$HARBOR_CLI_SIGN_ENV
RPROF
