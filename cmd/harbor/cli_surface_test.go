...
var cliSurface = map[string]surfaceEntry{
	...
	"lighthouse rotation-record": {why: "the scheduled cert-rotation timer records its own per-lighthouse run outcome (ok|skip|fail) into harbor so rotation liveness is observable via metrics — a local on-the-box write by the rotation job, not an operator/console action"},
	...
	"policy propose":           {why: "operator/bootstrap propose of a firewall policy via dual-control (ADR 0011 Phase 1 deleted the API's POST /policy/propose; the declarative PUT /admin/v1/config/policy is the day-to-day path). This CLI path is for bootstrap/break-glass before the API is up — it proposes a change a distinct operator approves with 'policy approve'"},
	"genesis":                  {why: "cluster bootstrap — runs offline before any server/CA exists"},
	"ca-init":                  {why: "CA / HSM bootstrap — offline key ceremony"},
	"ca list":                  {why: "local inspection of the CA-rotation lifecycle (M8, design §4.6). The console CA Rotation view (GET /admin/v1/ca) now shows this read-only; the CLI remains the break-glass path that works even when the API is down"},
	"ca adoption":              {why: "local inspection of CA-rotation trust-adoption progress (M8.1, design §4.6) — which live hosts have confirmed via heartbeat that they trust a staged CA before `ca activate` cuts over. The console surfaces this read-only (GET /admin/v1/ca/{id}/adoption); the CLI remains the break-glass/local path"},
	...
	"ca abandon":               {why: "security break-glass: cancel a staged CA that was never activated (M8, design §4.6); operator-driven, console surface lands in a later M8 slice"},
	"ca force-renew":           {why: "security break-glass: start/stop an accelerated drain of a draining CA (M8.3c, design §4.6) — force-renews its remaining leaf holders onto the active CA in waves so it can be retired without waiting a full cert lifetime; operator-driven, console surface lands in a later M8 slice"},
	"ca schedule-key-deletion": {why: "security break-glass: schedule a RETIRED CA's non-exportable signing key for deletion in KMS (M8.4, design §4.6) — a destructive, KMS-window-guarded custody action like ca-init/genesis; operator-driven, console surface lands in a later M8 slice"},
	"ca cancel-key-deletion":   {why: "security break-glass: abort a pending CA signing-key deletion during the KMS window (M8.4, design §4.6); operator-driven recovery action, console surface lands in a later M8 slice"},

	"config-key list":                  {why: "local inspection of the config-signing-key rotation lifecycle (M8.5, design §4.6/§4.8). The console Config-Key Rotation view (GET /admin/v1/config-key) shows this read-only; the CLI remains the break-glass path that works even when the API is down"},
	"config-key adoption":              {why: "local inspection of config-key-rotation trust-adoption progress (M8.5) — which live hosts confirm via heartbeat that they trust a staged config key before `config-key activate` cuts over. The console surfaces this read-only (GET /admin/v1/config-key/{id}/adoption); the CLI is the break-glass/local path"},
	"config-key stage":                 {why: "security break-glass: stage a new config-signing key into the trust set (M8.5, design §4.6/§4.8) — mint-K2 half of an operator-driven config-key rotation; handles a co-equal TCB-root public key; console surface is read-only by design (lifecycle actions stay break-glass)"},
	"config-key activate":              {why: "security break-glass: cut fleet bundle-signing over to a staged config key (M8.5) — a hard-gated (100% adoption) TCB-root transition run by an operator; console surface is read-only by design"},
	"config-key retire":                {why: "security break-glass: remove a drained config key from the trust set (M8.5), gated fail-closed on the live fleet fully trusting the active key; operator-driven, console surface read-only by design"},
	"config-key abandon":               {why: "security break-glass: cancel a staged config key that was never activated (M8.5); operator-driven, console surface read-only by design"},
	"config-key schedule-key-deletion": {why: "security break-glass: schedule a RETIRED config key's non-exportable signing key for deletion in KMS (M8.5/M8.4-style) — a destructive, KMS-window-guarded custody action; operator-driven, console surface read-only by design"},
	"config-key cancel-key-deletion":   {why: "security break-glass: abort a pending config-key signing-key deletion during the KMS window (M8.5); operator-driven recovery action, console surface read-only by design"},
	"issue-cert":               {why: "break-glass direct cert issuance (decision 10: CLI-only break-glass path)"},
	"backfill-cp-enrollment":   {why: "break-glass repair: record the missing issued enrollment row for a pre-fix genesis control-plane/lighthouse cert so the P10 revocation guard + renewal resolve it; abuse-guarded to control-plane rows only — like genesis/issue-cert, an offline/recovery action, never a console capability"},
	"cloudtrust publish":       {why: "operator/bootstrap publish of the cloud-trust config (two distinct operators, like genesis); the console's PUT /admin/v1/config/cloudtrust is the day-to-day path (ADR 0011 Phase 1), this is for bootstrap/break-glass before the API is up. Both converge on the config store via the dual-control committer"},
	"usertrust publish":        {why: "operator/bootstrap publish of the SSO user-trust config (ADR 0004; two distinct operators, like genesis); the console's PUT /admin/v1/config/usertrust is the day-to-day path (ADR 0011 Phase 1), this is for bootstrap/break-glass before the API is up. Both converge on the config store"},
	...
	"blocklist bulk-revoke":    {why: "security break-glass: dual-controlled, rate-limited mass revoke (7.2) — two distinct operators (propose + approve), like genesis/cloudtrust; must work when the console is down (mass-compromise response). The console bulk-revoke view lands in 7.2/UI-5"},
	...
	"nebula add":               {why: "register a nebula release in the distribution registry (ADR 0003 Phase 1c); CLI-only ingest — the console Releases page lists + stages but deliberately does NOT upload binaries (Harbor stays a pointer registry)"},
	"nebula add-artifact":      {why: "register an additional per-arch nebula binary for an existing generation (per-arch release URL support); CLI-only ingest, same rationale as `nebula add`"},
	"nebula list":              {op: "listReleases"},
	"nebula release":           {op: "startReleaseRollout"},
	"nebula status":            {op: "listReleases"}, // the releases view carries each lane's rollout status
	"nebula abort":             {op: "abortReleaseRollout"},
	"pilot add":                {why: "register a pilot release in the distribution registry (ADR 0003 Phase 3c); CLI-only ingest — the console Releases page lists + stages but deliberately does NOT upload binaries (Harbor stays a pointer registry)"},
	"pilot add-artifact":       {why: "register an additional per-arch pilot binary for an existing generation (per-arch release URL support); CLI-only ingest, same rationale as `pilot add`"},
	"pilot list":               {op: "listReleases"},
	"pilot release":            {op: "startReleaseRollout"},
	"pilot status":             {op: "listReleases"},
	"pilot abort":              {op: "abortReleaseRollout"},
